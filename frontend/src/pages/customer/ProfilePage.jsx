      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"36px 20px 100px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16, animation:"fadeUp 0.5s ease 0.1s both" }}>
          {[["📦","My Orders","/orders"],["📍","Addresses","/address"],["🛒","Catalog","/catalog"],["📊","Dashboard","/customer/dashboard"]].map(([icon,lbl,to])=>(
            <button data-magnetic key={to} onClick={()=>navigate(to)} style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 18px", background:tk.bgCard, border:`1.5px solid ${tk.border}`, borderRadius:14, cursor:"pointer", fontWeight:700, fontSize:14, color:tk.text, fontFamily:"'Inter',sans-serif", transition:"all 0.2s", textAlign:"left" }}
