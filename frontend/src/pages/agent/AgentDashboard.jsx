          ...
          <div>
            ...
            {recentOrders.length===0 ? (
              <div style={{ ...gc, borderRadius:20, padding:36, textAlign:"center" }}>
                ...
                <button onClick={()=>navigate("/agent/add-product")} style={{ background:"linear-gradient(135deg,#3b82f6,#1e40af)", color:"#fff", border:"none", padding:"11px 24px", borderRadius:50, cursor:"pointer", fontWeight:700, fontFamily:"'Inter',sans-serif", boxShadow:"0 4px 16px rgba(59,130,246,0.35)" }}>
                  Add First Product →
                </button>
              </div>
  ...
  return (
    <div style={{ background: dark?"rgba(12,22,15,0.82)":"rgba(255,255,255,0.82)", backdropFilter:"blur(24px)", WebkitBackdropFilter:"blur(24px)", border:`1px solid ${dark?"rgba(59,130,246,0.12)":"rgba(255,255,255,0.9)"}`, borderRadius:16, padding:"16px 18px", marginBottom:10, boxShadow: dark?"0 4px 20px rgba(0,0,0,0.4)":"0 4px 20px rgba(0,0,0,0.07)" }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={()=>respond("accepted")} disabled={!!loading} style={{ flex:1, padding:"8px", background:"linear-gradient(135deg,#059669,#047857)", color:"#fff", border:"none", borderRadius:50, cursor:"pointer", fontWeight:700, fontSize:13, fontFamily:"'Inter',sans-serif", boxShadow:"0 3px 10px rgba(5,150,105,0.3)", opacity:loading?0.7:1, transition:"all 0.2s" }}>
