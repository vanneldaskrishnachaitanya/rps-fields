      ...
      <div style={{ position: "relative", maxWidth: 1000, margin: "0 auto", paddingBottom: 80 }}>
        ...
        <div style={{ position: "relative", zIndex: 2, maxWidth: 840, margin: "0 auto" }}>
          <style>
            {`
              .wacus-block {
                color: ${dark ? "rgba(145, 156, 150, 0.45)" : "rgba(50, 72, 61, 0.58)"};
      ...
      <section style={{ padding:"clamp(10px,2vw,20px) var(--page-px,clamp(10px,2.2vw,24px)) clamp(18px,2.8vw,30px)", background:tk.bg }}>
        <div style={{ maxWidth:"var(--content-max)", margin:"0 auto", display:"grid", gridTemplateColumns:"minmax(280px,360px) 1fr", gap:14, alignItems:"start" }}>
          ...
          <div data-id="trust" className={dark ? "liquid-glass-dark" : "liquid-glass"} style={{
            ...
            <h3 style={{ color: dark ? "#fff" : "#173829", fontSize:"clamp(17px,1.8vw,22px)", fontFamily:"'Playfair Display',Georgia,serif", lineHeight:1.3, marginBottom:8, fontStyle:"italic" }}>
              "The freshest produce I've ever received, straight from the farm to my kitchen."
            </h3>
            <p style={{ color: dark ? "#74c69d" : "#216d49", fontSize:11.5, fontWeight:700, marginBottom:10 }}>— Priya M., Hyderabad customer since 2024</p>
            <div data-id="trust2" style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8, ...reveal("trust2", 0.12) }}>
              {[["🌾","500+","Verified Farmers"],["📦","10K+","Orders Delivered"],["⭐","4.8/5","Avg Rating"],["🗺","15+","States Covered"]].map(([icon,num,label]) => (
                <div key={label} style={{ textAlign:"center", padding:"6px 4px", borderRadius:10, background: dark ? "rgba(0,0,0,0.12)" : "rgba(27,67,50,0.08)" }}>
                  <div style={{ fontSize:15 }}>{icon}</div>
                  <div className="num" style={{ color: dark ? "#fff" : "#163527", fontSize:17, fontWeight:900, fontFamily:"'Inter',sans-serif" }}>{num}</div>
                  <div style={{ color: dark ? "rgba(255,255,255,0.62)" : "rgba(22,53,39,0.72)", fontSize:10 }}>{label}</div>
                </div>
              ))}
            </div>
          </div>
